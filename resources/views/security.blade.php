                        ...
                        <div class="p-12 flex flex-col lg:flex-row gap-12">
                            ...
                            <div class="flex-1 space-y-8">
                                ...
                                <form action="{{ route('security.change') }}" method="POST" class="space-y-4 max-w-md">
                                    @csrf
                                    {{-- Error dari pinBag --}}
                                    @if($errors->pinBag->any())
                                        <div class="bg-red-100 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm font-medium animate-pulse">
                                            {{ $errors->pinBag->first() }}
                                        </div>
                                    @endif
                                    {{-- Error validasi default (newPin.digits dll) --}}
                                    @if($errors->any() && !$errors->pinBag->any() && !$errors->passwordBag->any())
                                        <div class="bg-red-100 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm font-medium animate-pulse">
                                            {{ $errors->first() }}
                                        </div>
                                    @endif
                                    @if(session('pin_success'))
                                        <div class="bg-green-100 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm font-medium">
                                            {{ session('pin_success') }}
                                        </div>
                                    @endif
                                    <div>
                                ...
                                <div class="pt-8 mt-8 border-t border-gray-100">
                                    ...
                                    <form action="{{ route('security.password') }}" method="POST" class="space-y-4 max-w-md">
                                        @csrf
                                        @if($errors->passwordBag->any())
                                            <div class="bg-red-100 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm font-medium animate-pulse">
                                                {{ $errors->passwordBag->first() }}
                                            </div>
                                        @endif
                            ...
                            <div class="flex-1">
                                <div class="bg-red-50/30 border border-red-100 rounded-3xl p-10 h-full flex flex-col items-center text-center">
                                    ...
                                    @if(session('freeze_success'))
                                        <div class="bg-green-100 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm font-medium w-full mb-4">
                                            {{ session('freeze_success') }}
                                        </div>
                                    @endif
                                    @if(session('success') && !session('pin_success') && !session('password_success') && !session('freeze_success'))
                                        <div class="bg-green-100 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm font-medium w-full mb-4">
                                            {{ session('success') }}
                                        </div>
                                    @endif
