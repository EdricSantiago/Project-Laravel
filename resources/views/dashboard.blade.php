...
<div class="max-w-4xl mx-auto mt-10 px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div class="bg-blue-600 text-white rounded-2xl p-6 shadow">
            ...
            <p class="text-xl font-bold mt-1">{{ Auth::user()->account_number ?? Auth::user()->account->account_number ?? '-' }}</p>
        </div>
    ...
    <h3 class="text-lg font-bold text-gray-800 mb-4">Menu Layanan Bank</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        
        <div class="bg-white p-6 rounded-2xl shadow flex flex-col justify-between border-l-4 border-green-500">
            <div>
                <h4 class="text-md font-bold text-gray-800">Proteksi & Asuransi</h4>
                <p class="text-sm text-gray-500 mt-1">Kelola jaminan kesehatan dan proteksi finansial masa depanmu di sini.</p>
            </div>
            <div class="mt-4">
                <a href="{{ route('securities.index') }}" class="inline-block bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-green-700 transition">
                    Buka Asuransi &rarr;
                </a>
            </div>
        </div>

        <div class="bg-white p-6 rounded-2xl shadow flex flex-col justify-between border-l-4 border-amber-500">
            <div>
                <h4 class="text-md font-bold text-gray-800">Token Listrik PLN</h4>
                <p class="text-sm text-gray-500 mt-1">Isi ulang daya listrik rumahmu secara instan 24 jam non-stop.</p>
            </div>
            <div class="mt-4">
                <a href="{{ route('transactions.index') }}" class="inline-block bg-amber-500 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-amber-600 transition">
                    Beli Token Listrik &rarr;
                </a>
            </div>
        </div>
